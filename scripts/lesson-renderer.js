...
function renderHanzi(item) {
  ...
  return `<div class="hanzi hanzi-clickable">${item.chars.map(char => {
    if (!char.a && char.p) return `<span class="hanzi-char hanzi-static">
...
function renderPhraseVisual(item) {
  if (item.numberGrid?.length) {
    return `<div class="number-strip" role="group" aria-label="numbers 1 to 10">
      <div class="number-row number-pinyin-row">
        ${item.numberGrid.map(cell => `<button class="number-token" data-audio="${cell.audio}" data-label="${cell.hanzi}"><span>${escapeHTML(cell.pinyin)}</span></button>`).join("")}
      </div>
      <div class="number-row number-hanzi-row">
        ${item.numberGrid.map(cell => `<button class="number-token" data-audio="${cell.audio}" data-label="${cell.hanzi}"><b>${escapeHTML(cell.hanzi)}</b></button>`).join("")}
      </div>
      <div class="number-row number-arabic-row">
        ${item.numberGrid.map(cell => `<button class="number-token" data-audio="${cell.audio}" data-label="${cell.hanzi}"><em>${escapeHTML(cell.number)}</em></button>`).join("")}
      </div>
    </div>`;
  }
  if (item.weekdayGrid?.length) {
    return `<div class="weekday-panel">
      <b>${escapeHTML(textFor(item.weekdayTitle || { en:"Weekdays", ja:"曜日一覧", ko:"요일", zh:"星期一览" }))}</b>
      <div class="weekday-grid">
        ${item.weekdayGrid.map(cell => `<div class="weekday-chip">
          ${cell.audio ? soundButton(cell.audio, cell.hanzi, true) : ""}
          <button class="weekday-word" type="button" data-audio="${cell.audio}" data-label="${cell.hanzi}">
            <ruby><rb>${escapeHTML(cell.hanzi)}</rb><rt>${escapeHTML(cell.pinyin || "")}</rt></ruby>
          </button>
          <span>${escapeHTML(textFor(cell.meaning))}</span>
        </div>`).join("")}
      </div>
    </div>`;
  }
...
function renderPhraseExamples(item) {
  const languageLines = item.languageExamples ? (item.languageExamples[state.language] || item.languageExamples.en || []) : null;
  if (languageLines) {
    const languageHead = ({ zh: "🇨🇳 中文", ja: "🇯🇵 日本語", en: "🇬🇧 English", ko: "🇰🇷 한국어" })[state.language] || "🇬🇧 English";
    const cleanLabel = (label = "") => label.includes("·") ? label.split("·").slice(1).join("·").trim() : label;
    const rows = languageLines.filter(example => !String(example.label || "").startsWith("📚"));
    const refs = languageLines.filter(example => String(example.label || "").startsWith("📚"));
    const referenceTitle = ({ zh: "📚 参考文献", ja: "📚 参考文献", en: "📚 References", ko: "📚 참고문헌" })[state.language] || "📚 References";
    return `<div class="phrase-examples history-panel">
      <b class="history-title">${escapeHTML(textFor(item.examplesTitle || { en: "4-language historical introduction", ja: "4言語の歴史紹介", ko: "4개 언어 역사 소개", zh: "四语历史介绍" }))}</b>
      <div class="history-language">${escapeHTML(languageHead)}</div>
      <div class="history-rows">
        ${rows.map(example => `<div class="history-row">
          <div class="history-label">${escapeHTML(cleanLabel(example.label))}</div>
          <div class="history-text">${escapeHTML(example.text || example)}</div>
        </div>`).join("")}
      </div>
      ${refs.length ? `<div class="history-references">
        <b>${escapeHTML(referenceTitle)}</b>
        ${refs.map(ref => `<p>${escapeHTML(ref.text || ref)}</p>`).join("")}
      </div>` : ""}
    </div>`;
  }
  ...
  return `<div class="phrase-examples">
    ...
    ${item.examples.map(example => `<div class="phrase-example-line">
      ${example.audio ? soundButton(example.audio, textFor(example.text), true) : ""}
    ...
    <div class="phrase-card ${item.type === "builder" ? "builder-card" : ""} ${item.type === "builder" && item.optionGroups?.some(group => state.openSlot === builderSlotKey(index, item, group.slot)) ? "menu-open" : ""} ${index === state.phraseIndex ? "active" : ""}" data-phrase-index="${index}">
      <div class="phrase-main">
        <div class="phrase-copy">
          ...
          <div class="translation">${escapeHTML(item.type === "builder" ? builderPhraseTranslation(item, index) : textFor(item.translation))}</div>
          ${item.note ? `<div class="phrase-note"><span aria-hidden="true">💡</span>${escapeHTML(textFor(item.note))}</div>` : ""}
          ${renderPhraseExamples(item)}
          ${renderPhraseVisual(item)}
        </div>
...
function monthInfoFromISO(iso) {
  ...
  return {
    ...
    monthNumber,
    monthText,
    monthName: monthNames[monthNumber - 1] || "MONTH",
    daysInMonth: new Date(year, monthNumber, 0).getDate(),
    firstWeekday: new Date(year, monthNumber - 1, 1).getDay()
  };
}

function monthISO(year, monthNumber) {
  return `${year}-${String(monthNumber).padStart(2, "0")}-01`;
}

function shiftMonthISO(iso, offset) {
  const info = monthInfoFromISO(iso);
  return monthISO(info.year, info.monthNumber + offset);
}
...
function renderDayRail() {
  const relativeDays = [-1, 0, 1].map(offset => {
    const iso = localISODate(offset);
    const key = lessonKeyByISO(iso);
    if (!key) return null;
    const lesson = window.LESSONS[key];
    return { offset, iso, key, lesson, calendar: lesson.calendar, label: relativeDayLabel(offset) };
  }).filter(Boolean);

  // 如果自然日期落在没有更新课程的断档里，不显示空日期，改为显示当前课程附近的真实课程。
  const days = relativeDays.length ? relativeDays : orderedLessonKeys()
    .map(key => ({ key, lesson: window.LESSONS[key], iso: lessonDateISO(window.LESSONS[key]) }))
    .filter(item => item.iso)
    .sort((a, b) => a.iso.localeCompare(b.iso))
    .slice(Math.max(0, orderedLessonKeys().indexOf(state.lesson) - 1), Math.max(0, orderedLessonKeys().indexOf(state.lesson) - 1) + 3)
    .map(item => ({
      ...item,
      calendar: item.lesson.calendar,
      label: textFor(item.lesson.tag)
    }));

  $("#dayRail").innerHTML = days.map(({ iso, key, lesson, calendar, label }) => {
    const tag = textFor(lesson.tag);
    return `<button class="day-card ${key === state.lesson ? "active" : ""}" data-lesson="${key}" data-day-iso="${iso}">
      <span class="day-dot"></span>
      <span class="day-date"><b>${calendar.day}</b><small>${calendar.month}</small></span>
      <span class="day-copy"><b>${escapeHTML(label)}</b><small>${escapeHTML(tag)}</small></span>
    </button>`;
  }).join("");
...
function renderCalendarMenu(order = orderedLessonKeys()) {
  ...
  if (!menu) return;
  const selectedMonthISO = state.calendarMonth || monthISO(monthInfoFromISO(localISODate(0)).year, monthInfoFromISO(localISODate(0)).monthNumber);
  const currentMonth = monthInfoFromISO(selectedMonthISO);
  const lessons = order.map((key) => {
    const lesson = window.LESSONS[key];
    const iso = lessonDateISO(lesson);
    const [yearText, monthText, dayText] = iso.split("-");
    return { key, lesson, iso, year: Number(yearText), monthText, dayNumber: Number(dayText) };
  }).filter(item => item.year === currentMonth.year && item.monthText === currentMonth.monthText);
  const lessonByDay = new Map(lessons.map(item => [item.dayNumber, item]));
  const todayInfo = monthInfoFromISO(localISODate(0));
  const todayDay = Number(localISODate(0).split("-")[2]);
  const isCurrentRealMonth = todayInfo.year === currentMonth.year && todayInfo.monthNumber === currentMonth.monthNumber;
  menu.innerHTML = `
    <div class="calendar-mini-head month-tone-${currentMonth.monthNumber}">
      <span class="calendar-rings" aria-hidden="true"><i></i><i></i></span>
      <div class="calendar-month-nav">
        <button class="calendar-arrow" type="button" data-calendar-month="${shiftMonthISO(selectedMonthISO, -1)}" aria-label="Previous month">‹</button>
        <div class="calendar-date-title">
          <b>${currentMonth.year}</b>
          <span>${String(currentMonth.monthNumber).padStart(2, "0")}</span>
        </div>
        <button class="calendar-arrow" type="button" data-calendar-month="${shiftMonthISO(selectedMonthISO, 1)}" aria-label="Next month">›</button>
      </div>
      <small>${currentMonth.monthName} · ${ui().dailyTitle}</small>
    </div>
    ...
    <div class="calendar-day-grid">
      ${Array.from({ length: currentMonth.firstWeekday }, () => `<span class="calendar-pad" aria-hidden="true"></span>`).join("")}
      ${Array.from({ length: currentMonth.daysInMonth }, (_, dayIndex) => {
        ...
        return `<button class="calendar-day ${found ? "has-lesson" : "empty"} ${isCurrentRealMonth && day === todayDay ? "today" : ""} ${found?.key === state.lesson ? "active" : ""}" ${attrs}>
          <b>${day}</b>
  ...
  fitCalendarMenuToViewport();
}

function fitCalendarMenuToViewport() {
  const menu = $("#calendarMenu");
  if (!menu) return;
  menu.style.setProperty("--calendar-shift-x", "0px");
  if (!state.calendarOpen) return;

  // 窄版/手机版日历是 fixed 居中弹窗，不需要再做横向防出界偏移。
  // 否则切换月份重绘时，旧位置参与计算，反而会把浮窗推到屏幕外。
  ...
  requestAnimationFrame(() => {
    ...
    menu.style.setProperty("--calendar-shift-x", `${shift}px`);
  });
}
...
function renderLesson() {
  ...
  const displayLesson = state.dailyQuiz && !lesson.quiz ? activeQuizLesson() : lesson;
  ...
  const reviewButton = !lesson.quiz ? `<button class="daily-quiz-button ${state.dailyQuiz ? "active" : ""}" type="button" data-daily-quiz-start="true"><span>✓</span>${escapeHTML(ui().dailyQuiz)}</button>` : "";
  $("#lessonSwitch").innerHTML = `<span>${lessonDateISO(lesson) === localISODate(0) ? ui().todayTheme : ui().reviewTheme}</span><b>${textFor(lesson.tag)}</b>${reviewButton}`;
  if (displayLesson.quiz) {
    $("#phraseList").innerHTML = renderQuizLesson(displayLesson);
  } else {
    renderPhrases(lesson.phrases);
  }
  renderPinyin(lesson.pinyin);
  if (displayLesson.quiz) {
    $("#sideToolbox").innerHTML = `<div class="side-empty">${escapeHTML(textFor(displayLesson.summary))}</div>`;
  } else {
    renderSideToolbox();
  }
  renderDayRail();
