...
document.addEventListener("click", (event) => {
  if (event.target.closest("[data-daily-quiz-start]")) {
    if (!state.dailyQuiz) {
      state.dailyQuiz = true;
      resetQuizSession(activeQuizLesson());
    }
    renderLesson();
    return;
  }
  if (event.target.closest("[data-daily-quiz-exit]")) {
    state.dailyQuiz = false;
    renderLesson();
    return;
  }
  ...
  if (lesson) { state.lesson = lesson.dataset.lesson; state.dailyQuiz = false; state.phraseIndex = 0; state.calendarOpen = false; renderLesson(); }
...
$("#homeButton").addEventListener("click", () => { initializeCurrentLesson(); state.dailyQuiz = false; state.phraseIndex = 0; renderLesson(); });
