...
function quizBankFor(lesson) {
  const bankName = lesson.quiz?.bank || "weekend";
  if (bankName === "__daily") return dailyQuizBankFor(window.LESSONS[state.lesson]);
  const customBanks = window.QUIZ_BANKS || {};
  const banks = Object.assign({ weekend: window.WEEKEND_QUIZ_BANK || [] }, customBanks);
  const bank = banks[bankName] || banks.weekend || [];
  return (lesson.quiz?.questionIds || []).length
    ? bank.filter(item => lesson.quiz.questionIds.includes(item.id))
    : [...bank];
}

function quizMeaningText(lesson, meaning) {
  const forcedLanguage = lesson.quiz?.meaningLanguage;
  if (forcedLanguage && meaning?.[forcedLanguage]) return meaning[forcedLanguage];
  return textFor(meaning);
}

function quizSessionKey() {
  return state.dailyQuiz ? `${state.lesson}::daily` : state.lesson;
}

function dailyQuizItemFromPhrase(item, index) {
  const audioParts = item.audio ? null : (item.chars || []).map(char => char.a).filter(Boolean);
  if (!item.hanzi || !item.pinyin || !item.translation || (!item.audio && !audioParts?.length)) return null;
  return {
    id: `daily-${state.lesson}-phrase-${index}`,
    hanzi: item.hanzi,
    pinyin: item.pinyin,
    audio: item.audio,
    audioParts,
...
function dailyQuizItemFromExample(example, phraseIndex, exampleIndex) {
  if (!example.audio || !example.text) return null;
  const zhText = example.text.zh || example.text.en || textFor(example.text);
  const firstChunk = String(zhText).split("·")[0].trim();
  const [hanzi, ...rest] = firstChunk.split(/\s+/);
  return {
    id: `daily-${state.lesson}-example-${phraseIndex}-${exampleIndex}`,
    hanzi: hanzi || firstChunk,
    pinyin: rest.join(" ") || "",
    audio: example.audio,
...
function dailyQuizBankFor(sourceLesson) {
  const items = [];
  (sourceLesson.phrases || []).forEach((phrase, phraseIndex) => {
    const phraseItem = dailyQuizItemFromPhrase(phrase, phraseIndex);
    if (phraseItem) items.push(phraseItem);
    (phrase.examples || []).forEach((example, exampleIndex) => {
      const exampleItem = dailyQuizItemFromExample(example, phraseIndex, exampleIndex);
      if (exampleItem) items.push(exampleItem);
    });
  });
  const seen = new Set();
...
function dailyQuizLesson() {
  const sourceLesson = window.LESSONS[state.lesson];
  return {
    ...sourceLesson,
    quiz: { bank: "__daily", count: 3, meaningLanguage: state.language },
    summary: {
      en: "A tiny three-question review from today's lesson.",
      ja: "今日の内容から3問だけ復習しましょう。",
      ko: "오늘 내용에서 세 문제만 복습해요.",
      zh: "从今天的内容里抽三题小复习。"
    }
  };
}

function activeQuizLesson() {
  const lesson = window.LESSONS[state.lesson];
  return state.dailyQuiz && lesson && !lesson.quiz ? dailyQuizLesson() : lesson;
}
...
function resetQuizSession(lesson) {
  ...
  const key = quizSessionKey();
  const previousSession = state.quiz[key];
  ...
  return `<div class="quiz-wrap">
    <div class="quiz-card">
      ...
      <div class="quiz-options">
        ${current.options.map(option => {
          ...
          return `<button class="quiz-option ${stateClass}" data-quiz-option="${option.id}" ${current.correctAnswered ? "disabled" : ""}>${escapeHTML(quizMeaningText(lesson, option.meaning))}</button>`;
        }).join("")}
      </div>
    ...
    ${session.completed ? `<div class="quiz-complete">
      ...
      <div class="quiz-complete-card">
        ...
        <div>
          ${state.dailyQuiz ? `<button data-daily-quiz-exit="true">${escapeHTML(ui().backToLesson)}</button>` : `<button data-quiz-home="true">${escapeHTML(copy.home)}</button>`}
