...
function selectedOption(index, item, slot) {
  return (builderState(index, item)[slot] || [])[0];
}

function selectedOptionTranslation(index, item, slot) {
  const option = selectedOption(index, item, slot);
  return option?.translation ? textFor(option.translation) : cleanAudioText(option?.text || "");
}
...
function builderPhraseTranslation(item, index) {
  if (item.id !== "work") return textFor(item.translation);

  const workplace = selectedOptionTranslation(index, item, "workplace");
  const job = selectedOptionTranslation(index, item, "job");
  if (!workplace && !job) return textFor(item.translation);

  const workplaceText = workplace || "____";
  const jobText = job || "____";
  const translations = {
    en: `Hello everyone! I’m a ${jobText} at ${workplaceText}.`,
    ja: `みなさん、こんにちは。私は${workplaceText}の${jobText}です。`,
    ko: `여러분 안녕하세요! 저는 ${workplaceText}의 ${jobText}입니다.`,
    zh: `大家好！我是${workplaceText}的${jobText}。`
  };
